...
class If_Then_Else_If {
 public:
  ...
  constexpr If_Then<T> Then(const T& t_now) && {
    ...
    return (previous_condition) ? If(true).Then(std::move(previous_t))
                                : If(condition_now).Then(t_now);
  }

  constexpr If_Then<T> Then(T&& t_now) && {
    // written as single return statement for C++11 compatibility
    return (previous_condition) ? If(true).Then(std::move(previous_t))
                                : If(condition_now).Then(std::move(t_now));
  }

 private:
  constexpr If_Then_Else_If(bool _previous_condition, T&& _previous_t,
                            bool _condition_now)
      : previous_condition(_previous_condition),
        previous_t(std::move(_previous_t)),
        condition_now(_condition_now) {}
  constexpr If_Then_Else_If(bool _previous_condition, const T& _previous_t,
                            bool _condition_now)
      : previous_condition(_previous_condition),
        previous_t(_previous_t),
        condition_now(_condition_now) {}
...
class If_Then {
 ...
 private:
  constexpr If_Then(bool _condition, T&& _t)
      : condition(_condition), t(std::move(_t)) {}
  constexpr If_Then(bool _condition, const T& _t)
