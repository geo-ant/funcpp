...
//! helper bool that checks whether a callable returns void when invoked with
//! one argument
template <typename F, typename T>
constexpr bool invoke_result_is_void_v =
    std::is_same_v<void, std::invoke_result_t<F, T>>;
}  // namespace detail
...
template <typename T, typename F>
auto operator|(const std::optional<T> &arg, F &&func)
    -> std::enable_if_t<detail::invoke_result_is_void_v<F, T>,
